...
namespace ConsoleTester
{
	public class OperationsExperiments
	{
		...
		static private Link L(Link source, Link linker, Link target)
		{
			return Link.Create(source, linker, target);
		}

		static private Link CompileMatchingQueryExecutionOperation(Link matchingQuery)
		{
		...
		static private Link Eval(Link link, Link argumentLink)
		{
		...
		static private Link EvalWithReferenceToItself(Link link, Link argumentLink)
		{
			if (link.Source == Net.Link && link.Linker == _thatExactlyIs)
				return link.Target;
			else if (link.Source == _result && link.Linker == Net.Of)
		...
		static private Link CreateDefinitionFromLink(Link link)
		{
			return CreateDefinition(link.Source, link.Linker, link.Target);
		}

		static private Link CreateDefinition(Link source, Link linker, Link target)
		{
		...
		static private Link ExecuteLinksOperationsExecution(Link executionQuery, Link argument)
		{
		...
		static private Link ExecuteLinkOperationCore(Link operation, Link argument)
		{
		...
		static private Link ExecuteLinkCreation(Link creationQuery, Link argument)
		{
			Link definition = GetLinksDescriptionDefinition(creationQuery.Target);
			return ExecuteLinkCreationCore(definition, argument);
		}

		static private Link ExecuteLinkCreationCore(Link definition, Link argument)
		{
		...
		static private Link ExecuteLinkUpdate(Link updateQuery, Link argument)
		{
		...
		static private void ExecuteLinkDeletion(Link deletionQuery, Link argument)
		{
		...
		static private Link ExecuteLinkPartSelection(Link selectionQuery, Link argument)
		{
			Link definition = selectionQuery.Target;
			return ExecuteLinkPartSelectionCore(definition, argument);
		}

		static private Link ExecuteLinkPartSelectionCore(Link definition, Link argument)
		{
		...
		static private Link ExecuteLinkSelection(Link selectionQuery, Link argument)
		{
		...
		static private Link ExecuteLinkSelectionByDefinition(Link definition, Link argument)
		{
			Link sourceLink, linkerLink, targetLink;
		...
		static private Dictionary<Link, Func<Link, bool>> SelectionQueriesCheckers = new Dictionary<Link, Func<Link, bool>>();

		static private HashSet<Link> ExecuteLinksSelectionFromContext(Link selectionQuery, Link argument)
		{
		...
		static private Func<Link, bool> CreateSelectionChecker(Link selectionQuery)
		{
		...
		static private void EmitSelectionCheckerCore(ILGenerator il, Label exitLabel, List<string> levels, Link selectionQueryDefinition)
		{
			Link sourceLink, linkerLink, targetLink;
		...
		private static void EmitEqualityCheck(ILGenerator il, Label exitLabel, List<string> levels, Link standardLink, string nameOfStandardLinkProperty)
		{
			...
			il.EmitLiteralLoad(standardLink.GetPointer());
		...
		static private HashSet<Link> ExecuteLinksSelection(Link selectionQuery, Link argument)
		{
		...
		static private HashSet<Link> ExecuteLinksSelectionCore(Link definition, Link argument)
		{
		...
		{
			static public SetsCountComparer Default = new SetsCountComparer();
		...
		static private HashSet<Link> CollectParamLinkReferers(Link param, HashSet<Link> paramValues)
		{
			HashSet<Link> result = new HashSet<Link>();
		...
		static private HashSet<Link> CollectParamLinkReferers(Link param)
		{
			HashSet<Link> result = new HashSet<Link>();
		...
		static private long GetParamLinkReferersCount(Link param)
		{
			if (param.Target == _sourceLink)
		...
		static private void GetLinks(Link sequence, out Link sourceLink, out Link linkerLink, out Link targetLink)
		{
		...
		static private Link GetLinksDescriptionDefinition(Link link)
		{
		...
		static private bool IsLinkDescriptionParamSourceSpecific(Link paramSource)
		{
